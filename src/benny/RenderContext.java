...
public class RenderContext extends Bitmap
{
	...
	private void FillTriangle(Vertex v1, Vertex v2, Vertex v3, Bitmap texture)
	{
		Matrix4f screenSpaceTransform = 
				new Matrix4f().InitScreenSpaceTransform(width()/2, height()/2);
	...
	{
		int xMin = (int)FastMath.ceil(left.GetX());
		int xMax = (int)FastMath.ceil(right.GetX());
...
//		float depthXStep = (right.GetDepth() - left.GetDepth())/xDist;
		...
		float lightAmt = left.GetLightAmt() + lightAmtXStep * xPrestep;
                ...
                final float twidth = (float)(texture.width() - 1);
                final float theight = (float)(texture.height() - 1);
		...
		for(int i = xMin; i < xMax; i++)
		{
			...
			if(depth < m_zBuffer[index])
			{
				...
				int srcX = (int)((texCoordX / oneOverZ) * twidth + 0.5f);
				int srcY = (int)((texCoordY / oneOverZ) * theight + 0.5f);

				copyWithLight(i, j, srcX, srcY, texture, lightAmt);
                                
//	public void copy(int destX, int destY, int srcX, int srcY, Bitmap src, float lightAmt)
//	{
