...
public class Gradients
{
			...
			Vertex maxYVert, float oneOverdX)
	{
		return
			...
			((values[0] - values[2]) *
			(midYVert.y() - maxYVert.y()))) * oneOverdX;
	}
			...
			Vertex maxYVert, float oneOverdY)
	{
		return
			...
			((values[0] - values[2]) *
			(midYVert.x() - maxYVert.x()))) * oneOverdY;
	}
	...
	public Gradients(Vertex minYVert, Vertex midYVert, Vertex maxYVert) {
            ...
            set(minYVert, midYVert, maxYVert);
        }
	...
	public Gradients set(Vertex minYVert, Vertex midYVert, Vertex maxYVert)
	{
		float oneOverdX = 1.0f /
			(((midYVert.x() - maxYVert.x()) *
			(minYVert.y() - maxYVert.y())) -
			((minYVert.x() - maxYVert.x()) *
			(midYVert.y() - maxYVert.y())));

		float oneOverdY = -oneOverdX;

                

		m_depth[0] = minYVert.pos().z();
		m_depth[1] = midYVert.pos().z();
		m_depth[2] = maxYVert.pos().z();
		...
		m_oneOverZ[0] = 1.0f/minYVert.pos().w();
		m_oneOverZ[1] = 1.0f/midYVert.pos().w();
		m_oneOverZ[2] = 1.0f/maxYVert.pos().w();

		m_texCoordX[0] = minYVert.GetTexCoords().x() * m_oneOverZ[0];
		m_texCoordX[1] = midYVert.GetTexCoords().x() * m_oneOverZ[1];
		m_texCoordX[2] = maxYVert.GetTexCoords().x() * m_oneOverZ[2];

		m_texCoordY[0] = minYVert.GetTexCoords().y() * m_oneOverZ[0];
		m_texCoordY[1] = midYVert.GetTexCoords().y() * m_oneOverZ[1];
		m_texCoordY[2] = maxYVert.GetTexCoords().y() * m_oneOverZ[2];
