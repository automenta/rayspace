...
import unused.terrain.JavaRenderer.MyMath;
...
public class Terrain extends Drawable
{
	...
	public void RandomizeFieldHeight(double min, double max, double maxChange)
	{
		...
		this.vertices = new Tri[(this.SizeX - 1) * (this.SizeY - 1) * 2];
	...
	public LinkedList<Tri> getVertices()
	{
		LinkedList<Tri> ll = new LinkedList<Tri>();
		...
		for (int x = 0; x < this.FieldHeight.length; x++)
		{
			for (int y = 0; y < this.FieldHeight[x].length; y++)
			{
				...
				if (x != 0 && y != 0)
				{
					ll.add(new Tri(points[x][y], points[x][y - 1], points[x - 1][y - 1]));
					ll.add(new Tri(points[x][y], points[x - 1][y - 1], points[x - 1][y]));
				}
			}
		}
