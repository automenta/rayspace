...
public class Renderer implements Runnable, IRenderer {
    ...
    public void render(long time) {
        ...
        for (SceneObject so : this.scene.drawables) {
            ...
            List<Tri> vertices = transformVertices((Drawable) so, t, c);
    ...
    private void sortVertices(List<Tri> vertices) {
        // sort vertices of drawable
        vertices.sort(new Comparator<Tri>() {
            @Override
            public int compare(Tri arg0, Tri arg1) {
    ...
    private List<Tri> transformVertices(Drawable drawable, Transformation t, Camera c) {
        List<Tri> vertices = new ArrayList<Tri>(drawable.vertices.length);
        Vector3D delta = drawable.getPosition().clone().subtract(c.getPosition());
        for (Tri v : drawable.vertices) {
            Tri vc = new Tri(v.A.clone(), v.B.clone(), v.C.clone());
            if (t != null) {
                vc.transform(t);
            }
            vc.A.add(delta);
    ...
    private void project(Drawable drwbl, Tri v) {
        Color c = Color.black;
    ...
    private void project(Tri v, Color color, DrawMode fill) {
        ...
        Tri projection = new Tri(vectors[0], vectors[1], vectors[2]);
